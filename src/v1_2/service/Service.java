...
import v1_2.service.model.StatusCode;
 ...
 */
@Path("/v1.2/{projectName}/{tableName}")
public class Service {
    ...
    public ResultModel getAll() {
        ...
        //判断是否有逻辑删除
        if (tableMap.get("useDelFlag") != null) {
            deleteFlag = null;
        }
        ...
        if (isCollect) {
            ...
            try {
                if (map.containsKey("pageIndex")) {
                    ...
                    String pageSql = "select count(*) " + sql.substring(sql.indexOf("from"), sql.length());
                    ResultSet countRs = jdbcHelper.executeResultSet(pageSql, paramsMap.values().toArray());
    ...
    * */
    ...
    public ResultModel delete(@PathParam("pkey") String pkey) {
        ...
        StringBuilder kafkaSql = new StringBuilder();
        ...
        if (deleteFlag == null || (tableMap.get("useDelFlag") != null && !((Boolean) tableMap.get("useDelFlag")))) {
            ...
            if (!isDefault) {
                kafkaSql.append(String.format("delete from %s where %s='%s'", tableName, pk, pkey));
            } else {
                kafkaSql.append(String.format("delete from %s where %s=%s", tableName, pk, pkey));
            }
        } else {
            ...
            if (!isDefault) {
                kafkaSql.append(String.format("update %s set %s=1 where %s='%s'", tableName, deleteFlag, pk, pkey));
            } else {
                kafkaSql.append(String.format("update %s set %s=1 where %s=%s", tableName, deleteFlag, pk, pkey));
            }
        }
        int res = jdbcHelper.executeUpdate(sql.toString(), pkey);
        if (res > 0) {
            msg = "success";
            statusCode = StatusCode.SUCCESS;
        } else {
        ...
        resultModel.setCode(statusCode.toString());
        kafkaProducer.produce(projectName, request.getRequestURI(), tableName, "", request.getMethod(), kafkaSql.toString(), request.getRemoteAddr(), resultModel.getCode(), getTimespan());
        return resultModel;
    }
    ...
    public ResultModel insert(String jsonStr) {
        ...
        if (jsonStr.contains("content")) {
            ...
            jsonStr = json.getString("content");
        }
        //处理批量插入操作
        ...
        try {
            ...
            System.out.println("没有多层JSON数组");
        }
        for (Object jsonO : array) {
            String str = jsonO.toString();
            Map<String, Object> map;
            try {
                map = Util.parseJSON2Map(str);
            } catch (Exception ex) {
            ...
            HashMap<FieldModel, Object> tmpMap = new HashMap<FieldModel, Object>();
            ...
            while (keyIt.hasNext()) {
                ...
                for (FieldModel fieldModel : fs) {
                    if (fieldModel.getName().equals(key)) {
                        tmpMap.put(fieldModel, map.get(key));
                    }
                }
            }
            //判断数据库中所必须的字段是否全部都有
            ...
            for (FieldModel fieldModel : fs) {
                if (fieldModel.getRequired()) {
                    if (!(tmpMap.containsKey(fieldModel))) {
            ...
            if (isCollect) {
                ...
                for (FieldModel o : tmpMap.keySet()) {
                    valueTmp += "?,";
                    fields += o.getName() + ",";
    ...
    public ResultModel insert(MultivaluedMap<String, String> map) {
        ...
        //处理意外过来的JSON数组
        if (map.containsKey("content") && map.keySet().size() < 2) {
            String url = request.getRequestURL().toString();
            Client c = Client.create();
            WebResource r = c.resource(url);
            String json = map.get("content").get(0);
            ClientResponse response = r.accept(MediaType.APPLICATION_JSON).type(MediaType.TEXT_PLAIN).post(ClientResponse.class, json);
            String result = response.getEntity(String.class);
            Map resultMap = Util.parseJSON2Map(result);
            JSONArray resArray = JSONArray.fromObject(resultMap.get("content").toString());
            //获得返回的新增的ID
            for (int i = 0; i < resArray.size(); i++) {
                IdentityHashMap<String, String> mapModel = new IdentityHashMap<String, String>();
                mapModel.put(pk, Util.parseJSON2Map(resArray.getString(i)).get(pk).toString());
                resultModel.getContent().add(mapModel);
            }
            resultModel.setCode(StatusCode.SUCCESS.toString());
            resultModel.setMessage("success");
        } else {
            //请求参数的键集合
            Iterator keyIt = map.keySet().iterator();
            HashMap<FieldModel, String> tmpMap = new HashMap<FieldModel, String>();
            //循环键集合，如果数据库字段中有包含这个参数信息就加入到tmpMap中
            //此步骤主要是过滤请求参数中的杂乱字段信息，最后到tmpMap中的参数都是和数据库中字段对应的
            while (keyIt.hasNext()) {
                String key = (String) keyIt.next();
                for (FieldModel fieldModel : fs) {
                    if (fieldModel.getName().equals(key)) {
                        tmpMap.put(fieldModel, map.get(key).get(0));
                    }
                }
            }
            //判断数据库中所必须的字段是否全部都有
    ...
    public ResultModel update(@PathParam("pkey") String pkValue, String jsonStr) {
        ...
        //数据库字段集合
        ArrayList<FieldModel> fs = Util.getfields(tableMap);
        HashMap<FieldModel, String> fieldMap = new HashMap<FieldModel, String>();
        for (FieldModel f : fs) {
            if (map.keySet().contains(f.getName())) {
                fieldMap.put(f, map.get(f.getName()).toString());
            }
        }
        ...
        if (map.size() < 1) {
            ...
            for (FieldModel key : fieldMap.keySet()) {
                sql += key.getName() + "=?,";
                if (!key.getType().equals("int")) {
                    kafkaSql += key.getName() + "='" + fieldMap.get(key) + "',";
                } else {
                    kafkaSql += key.getName() + "=" + fieldMap.get(key) + ",";
                }
            }
